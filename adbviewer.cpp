    ...
    if (!appendText.contains(devicesList_combobox->currentText()))
        {
            ...
            //sometimes I catch some bug when no data columns
            //not sure what's input but suppose this will handle out this
            if (totalCPUBuffer.count(" ")!=totalCPUcolumnCounter)
                {
                    totalCPUBuffer = QString(" ").repeated(totalCPUcolumnCounter);
                }
